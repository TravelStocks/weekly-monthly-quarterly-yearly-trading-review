...
const accountDays = [
  { date: "2026/6/22", weekday: "周一", note: "账户/交易待补" },
  { date: "2026/6/23", weekday: "周二", note: "无截图成交，冰点期观察" },
  { date: "2026/6/24", weekday: "周三", note: "诺德止损，切入海欣/大唐" },
  { date: "2026/6/25", weekday: "周四", note: "卖海欣/大唐，买亨通系" },
  { date: "2026/6/26", weekday: "周五", note: "账户与收盘复盘待补" },
];
...
async function main() {
  ...
  const closedRate = (totals.realized / previousWeekEquity) * 100;
...
<section class="two-col"><article class="panel" id="account"><h2>账户口径待补</h2><p>目前缺少 6/26 收盘账户截图，所以本页不强行写完整账户收益率。先锁定可核算部分：上一周页面期末权益为 ${money(previousWeekEquity)}；本周已平仓票闭环约 ${totals.realized >= 0 ? "+" : ""}${money(totals.realized)}；6/25 截图期末现金约 ${money(finalCash)}；期末持仓成本口径约 ${money(openCost)}，成本口径权益约 ${money(costBasisEquity)}。正式账户盈亏以后续账户截图为准。</p><div class="mini-grid"><span>上周期末权益 <b>${money(previousWeekEquity)}</b></span><span>闭环盈亏 <b class="${totals.realized >= 0 ? "pos" : "neg"}">${totals.realized >= 0 ? "+" : ""}${money(totals.realized)}</b></span><span>闭环比例 <b class="${closedRate >= 0 ? "pos" : "neg"}">${pct(closedRate)}</b></span><span>期末现金 <b>${money(finalCash)}</b></span><span>持仓成本 <b>${money(openCost)}</b></span><span>成本口径权益 <b>${money(costBasisEquity)}</b></span></div><div class="account-days">${accountDays.map((day) => `<div class="account-day"><strong>${esc(day.date)} ${esc(day.weekday)}</strong><span>状态 <b>${esc(day.note)}</b></span><span>账户 <b>待补</b></span><span>仓位 <b>待补</b></span><span>总额 <b>待补</b></span></div>`).join("")}</div></article><article class="panel"><h2>先版结论</h2><ul class="takeaways"><li>本周最大问题不是交易频率，而是诺德股份从上一周浮盈/核心预期转成大额止损，单票闭环约 -2,948.55。</li><li>海欣股份隔日止损约 -278.53，大唐发电基本打平，说明 6/24 的切仓方向还没有形成稳定赚钱贡献。</li><li>6/25 市场强度重新聚焦芯片、通信、元器件、算力，买入亨通光电和亨通股份，方向回到更强主线。</li><li>下一版最需要补：6/26 亨通系持仓去留、最终账户总额，以及你对“止损旧核心后是否应该立刻重开仓”的二次反思。</li></ul></article></section>
<section class="two-col"><article class="panel"><h2>闭环贡献</h2><p class="section-note">只统计已平仓且能由交割单/上一周期末成本推算的部分；亨通光电与亨通股份未平仓，暂不计入闭环。</p><div class="bar-list">${closedStats.map((s) => {
  ...
  }).join("")}</div></article><article class="panel"><h2>期末持仓推算</h2><p class="section-note">截至 6/25 截图，期末持仓为亨通光电 100 股、亨通股份 600 股。这里先用成交含费成本展示，不推断浮动盈亏；6/26 收盘后再用账户截图或持仓截图覆盖正式市值。</p><div class="mini-grid"><span>亨通光电 <b>100 股</b></span><span>亨通光电成本 <b>${money((stats.find((s) => s.code === "600487") || {}).openCost || 0)}</b></span><span>亨通股份 <b>600 股</b></span><span>亨通股份成本 <b>${money((stats.find((s) => s.code === "600226") || {}).openCost || 0)}</b></span><span>持仓成本合计 <b>${money(openCost)}</b></span><span>现金余额 <b>${money(finalCash)}</b></span><span>浮动盈亏 <b>待补</b></span><span>正式仓位 <b>待补</b></span></div></article></section>
...
<section class="panel" id="todo"><h2>缺口清单</h2><div class="todo-grid"><article class="todo-card"><h3>还需要你补</h3><ul><li>6/22、6/26 是否有成交或个人复盘。</li><li>6/26 收盘账户截图、持仓截图和最终权益。</li><li>本周二次反思：诺德为何拖到 6/24、海欣/大唐为何切入、亨通系的持仓预期。</li></ul></article><article class="todo-card"><h3>已完成</h3><ul><li>截图中 6/24-6/25 的 8 笔成交已录入。</li><li>诺德、海欣、大唐三个已闭环标的已先核算。</li><li>亨通光电、亨通股份期末成本口径已记录。</li><li>6/23 与 6/25 市场/板块/热榜数据已纳入逐日复盘。</li><li>成交明细已隐藏合同号、成交编号。</li></ul></article><article class="todo-card"><h3>下版优先更新</h3><ul><li>用正式账户截图替换闭环口径。</li><li>补 6/26 持仓去留和浮动盈亏。</li><li>把临时规则改成你的最终交易纪律。</li></ul></article></div></section>
</div></main></body></html>`;
