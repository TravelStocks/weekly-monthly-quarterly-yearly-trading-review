...
function renderScoresSection() {
  ...
  return `<section class="panel" id="scores"><h2>五项评分</h2><p class="section-note">评分服务于审判，不做安慰分；优先级固定为：风控质量 > 定龙质量 > 执行纪律 > 买卖点质量 > 仓位集中度。即便账户赚钱，只要利润来自非系统标的，系统分也要低。</p><div class="score-grid">${scores.map(([name, score, note, cls]) => `<article class="score-card ${cls}"><div><h3>${name}</h3><strong>${score}</strong></div><p>${note}</p></article>`).join("")}</div></section>`;
}

function renderCoreTicketSection() {
  const rows = [
    ["月前参考：神剑股份", "神剑股份", "神剑股份", "是", "已抓住第一唯一性", "定龙正确，贡献突出，可复制。"],
    ["5月主正样本：通鼎互联", "通鼎互联", "通鼎互联", "是", "已抓住，但后排亏损稀释利润", "唯一明确大幅正贡献，说明主线核心嗅觉在线。"],
    ["大唐方向", "大唐发电", "大唐电信 / 大连热电", "否", "被后排吸引，题材热被误当个股地位", "做了后排替代，确定性不足。"],
    ["华能/电力方向", "阶段第一辨识度核心", "华电辽能 / 华能蒙电 / 华电能源", "否", "非唯一中高位左侧，止损后回补/亏损后加单", "跟风与非唯一中高位反复亏损，且出现红线动作。"],
    ["5月后段修复：达实智能", "达实智能", "达实智能", "是", "回到核心处理，但仍需防扩散", "核心处理较有效，但不能扩散到弱票。"],
    ["月后参考：大有能源", "大有能源", "大有能源", "是", "第一性确认后持有", "第一性和唯一性验证后，持仓浮盈放大利润。"],
  ];
  return `<section class="panel" id="core"><h2>本月真正应该做的核心票</h2><p class="section-note">轻量定龙判断只判“是否围绕第一唯一性”，不展开五维打分；无明确核心的阶段只允许轻仓套利，不能硬造龙头。</p><div class="table-wrap"><table class="audit-table"><thead><tr><th>阶段</th><th>真正应该盯的核心</th><th>实际交易</th><th>定龙是否正确</th><th>错过/偏离原因</th><th>审判</th></tr></thead><tbody>${rows.map(([phase, core, actual, ok, reason, note]) => `<tr><td>${phase}</td><td><strong>${core}</strong></td><td>${actual}</td><td><span class="chip ${ok === "是" ? "pos" : "neg"}">${ok}</span></td><td>${reason}</td><td>${note}</td></tr>`).join("")}</tbody></table></div></section>`;
}
